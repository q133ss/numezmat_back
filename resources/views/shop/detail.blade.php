        ...
        <div class="container">
            <div class="rating-show-content">
                ...
                <div class="rating-show-control">
                    @can('edit-catalog')
                        <a href="{{route('catalog.edit', $product->id)}}" class="rating-show-edit-btn">Редактировать тему <img src="/assets/img/Edit_fill.png" alt=""></a>
                    @endcan

                    @can('block-catalog')
                        @if($product->is_block == 0)
                            <a href="{{route('catalog.block', ['id' => $product->id, 'action' => 1])}}" class="rating-show-block-btn">Заблокировать тему</a>
                        @else
                            <a href="{{route('catalog.block', ['id' => $product->id, 'action' => 0])}}" class="rating-show-block-btn comment-form-btn">Разблокировать тему</a>
                        @endif
                    @endcan
                </div>
