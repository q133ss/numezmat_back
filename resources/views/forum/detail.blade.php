            ...
            <div class="page-header-wrap">
                <div class="page-header-left">
                    <ul class="breadcrumbs">
                        ...
                        @endforeach
                        <li><a href="{{route('forum.show', $forum->category->id)}}">{{$forum->category->name}}</a></li>
                        <li>{{$forum->title}}</li>
                    </ul>
...
{{--                        Просмотры:--}}
{{--                        <span class="rating-show-views-count">{{$forum->views}}</span>--}}
{{--                    </div>--}}
                    ...
                    <div class="rating-show-bottom">
                        ...
                        <div class="rating-author">
                            Автор <a href="{{route('users.show', $forum->author->id)}}">{{$forum->author->name}}</a>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="rating-show-control">
                @can('edit-rating')
                    <a href="{{route('forum.edit', $forum->id)}}" class="rating-show-edit-btn">Редактировать тему <img src="/assets/img/Edit_fill.png" alt=""></a>
                @endcan

                @can('block-rating')
                    @if($forum->is_block == 0)
                        <a href="{{route('forum.block', ['id' => $forum->id, 'action' => 1])}}" class="rating-show-block-btn">Заблокировать тему</a>
                    @else
                        <a href="{{route('forum.block', ['id' => $forum->id, 'action' => 0])}}" class="rating-show-block-btn comment-form-btn">Разблокировать тему</a>
                    @endif
                @endcan
            </div>
            ...
            <div class="rating-show-wrapper">
                <div class="rating-show-content">
                    ...
                    <div class="comment-area display-n">
                        @if(Auth()->check())
                            ...
                            <form action="{{route('comment.send', ['type' => 'forum','post_id' => $forum->id])}}" method="POST" class="comment-form">
                                @csrf
                    ...
                    @include('includes.comments', ['comments' => $forum->comments, 'type' => 'forum', 'postId' => $forum->id])
                ...
                <div class="rating-show-sidebar">
                    <div class="rating-show-sidebar-wrap">
                        ...
                        @foreach($forum->relatedPosts() as $post)
                            <div class="sidebar-other-theme">
                                ...
                                <a href="{{route('forum.detail', $post->id)}}" class="other-theme-btn">Подробнее</a>
                            </div>
                        @endforeach
                    </div>
                </div>
